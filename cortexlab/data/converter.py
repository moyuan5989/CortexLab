"""Dataset format converters for CortexLab.

Converts HuggingFace dataset columns to CortexLab standard format
...
if TYPE_CHECKING:
    from cortexlab.data.catalog import DatasetProfile


def convert_dataset(hf_dataset, profile: "DatasetProfile") -> list[dict]:
    """Convert HF dataset to CortexLab standard format (raw, pre-tokenization).
...
def _convert_rename(hf_dataset, mapping: dict) -> list[dict]:
    """Simple column rename: map source columns to CortexLab standard names.
...
def _convert_text(hf_dataset, mapping: dict) -> list[dict]:
    """Convert a single text column to CortexLab text format."""
