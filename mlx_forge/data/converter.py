"""Dataset format converters for MLX Forge.

Converts HuggingFace dataset columns to MLX Forge standard format
...
if TYPE_CHECKING:
    from mlx_forge.data.catalog import DatasetProfile


def convert_dataset(hf_dataset, profile: "DatasetProfile") -> list[dict]:
    """Convert HF dataset to MLX Forge standard format (raw, pre-tokenization).
...
def _convert_rename(hf_dataset, mapping: dict) -> list[dict]:
    """Simple column rename: map source columns to MLX Forge standard names.
...
def _convert_text(hf_dataset, mapping: dict) -> list[dict]:
    """Convert a single text column to MLX Forge text format."""
